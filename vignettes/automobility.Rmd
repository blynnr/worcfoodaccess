...
library(tidyverse)
library(tidycensus)
library(tigris) # what is this for??
```
...
automob <- left_join(cbg_ejpop_d %>% 
              ...
              as.data.frame(), #%>% select(-geometry), # why is the geo column called Shape instead of geometry? why is "geometry type" for one geometry and for buildcbg multipoint?
                       by = "GEOID10") %>%
  as_tibble %>% st_as_sf() %>% 
